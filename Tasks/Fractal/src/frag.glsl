...
void main()
{
  ...
  Z = (gl_FragCoord.xy * vec2(FractW, FractH) - vec2(Mx, -My)) * 4.0 / vec2(FrameW, FrameH) - vec2(2.0, 2.0);
  ...
  o_color = vec4(float(n) * cos(u_time) / 255.0, float(n) * cos(u_time) * sin(u_time) / 255.0, float(n) * sin(u_time) / 255.0, 1) ;
